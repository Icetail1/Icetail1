...
function calcScore( enc_time ) {
	...
	for ( var idx = 0; idx < 11; idx ++ ) {
        ...
        if (idx === 0 && !usef_sub.includes("gus-atkper") ){
			...
			max_score = max_score + substat_max_tmp
		}
        ...
        if (idx === 5 && !usef_sub.includes("gus-critdmg") ){
			...
			max_score = max_score + Math.floor(substat_max_tmp * 1.1);
		}
        if (idx === 6 && !usef_sub.includes("gus-critch") ){
            tmp = 0;
        }else{
			max_score = max_score + Math.floor(substat_max_tmp * 1.5);
		}
        if (idx === 7 && !usef_sub.includes("gus-spd") ){
            tmp = 0;
        }else{
			max_score = max_score + Math.floor(substat_max_tmp * 2);
		}
        if (idx === 8 && !usef_sub.includes("gus-atkflat") ){
            tmp = 0;
        }else{
			max_score = max_score + Math.floor(substat_max_tmp / 10 );
		}
        if (idx === 9 && !usef_sub.includes("gus-defflat") ){
            tmp = 0;
        }else{
			max_score = max_score + Math.floor(substat_max_tmp / 7 );
		}
        if (idx === 10 && !usef_sub.includes("gus-hpflat") ){
            tmp = 0;
        }else{
			max_score = max_score + Math.floor(substat_max_tmp / 50 );
		}
