...
function resetplaceholder() {
	...
	if ( isReforged() ) {
		...
		document.getElementById("atkper").placeholder = "0% - " + ( substate_max[0] + reforge[0] ) + "%";
		document.getElementById("defper").placeholder = "0% - " + ( substate_max[1] + reforge[1] ) + "%";
		document.getElementById("hpper").placeholder = "0% - " + ( substate_max[2] + reforge[2] ) + "%";
		document.getElementById("eff").placeholder = "0% - " + ( substate_max[3] + reforge[3] ) + "%";
		document.getElementById("res").placeholder = "0% - " + ( substate_max[4] + reforge[4] ) + "%";
		document.getElementById("critdmg").placeholder  = "0% - " + ( substate_max[5] + reforge[5] ) + "%";
		document.getElementById("critch").placeholder  = "0% - " + ( substate_max[6] + reforge[6] ) + "%";
		document.getElementById("spd").placeholder = "0 - " + ( substate_max[7] + reforge[7] );
		document.getElementById("atkflat").placeholder = "0 - " + ( substate_max[8] + reforge[8] );
		document.getElementById("defflat").placeholder = "0 - " + ( substate_max[9] + reforge[9] );
		document.getElementById("hpflat").placeholder = "0 - " + ( substate_max[10] + reforge[10] );
	}
	else {	
		document.getElementById("atkper").placeholder = "0% - " + substate_max[0] + "%";
		document.getElementById("defper").placeholder = "0% - " + substate_max[1]+ "%";
		document.getElementById("hpper").placeholder = "0% - " + substate_max[2] + "%";
		document.getElementById("eff").placeholder = "0% - " + substate_max[3] + "%";
		document.getElementById("res").placeholder = "0% - " + substate_max[4] + "%";
		document.getElementById("critdmg").placeholder  = "0% - " + substate_max[5] + "%";
		document.getElementById("critch").placeholder  = "0% - " + substate_max[6] + "%";
		document.getElementById("spd").placeholder = "0 - " + substate_max[7];
		document.getElementById("atkflat").placeholder = "0 - " + substate_max[8];
		document.getElementById("defflat").placeholder = "0 - " + substate_max[9];
		document.getElementById("hpflat").placeholder = "0 - " + substate_max[10];
	}
...
function validate() {
	...
	if ( totaldata < getRequiredDataCount() ) {
		if ( g_lang === 'tw' )
			...
			err( "错误: 此装备要求至少 " + getRequiredDataCount() + " 项属性" );
		else
...
function calcScore( enc_time ) {
	...
	for ( var idx = 0; idx < 11; idx ++ ) {
		...
		// so we use 4 as maximum to calculate the score 
			...
			if (idx === 5  ){
				tmp = Math.floor(data[idx] * 1.1);
			}
			if (idx === 6  ){
				tmp = Math.floor(data[idx] * 1.5);
			}
			if (idx === 7 ){
				tmp = Math.floor(data[idx] * 2);
			}
			if (idx === 8 ){
				tmp = Math.floor(data[idx] / 10 );
			}
			if (idx === 9 ){
				tmp = Math.floor(data[idx] / 7);
			}
			if (idx === 10 ){
				tmp = Math.floor(data[idx] / 50);
			}
...
function report( enc_time, score ) {
	...
	for ( var idx = 0; idx < 11; idx ++ ) {
		if (idx === 0 && usef_sub.includes("gus-atkper") ){
			max_score = max_score + substat_max[idx];
			continue;
        }
        if (idx === 1 && usef_sub.includes("gus-defper") ){
            max_score = max_score + substat_max[idx];
        ...
        if (idx === 5 && usef_sub.includes("gus-critdmg") ){
			max_score = max_score + Math.floor(substat_max[idx] * 1.1);
			continue;		
		}
        if (idx === 6 && usef_sub.includes("gus-critch") ){
			max_score = max_score + Math.floor(substat_max[idx] * 1.5);
			continue;
		}
        if (idx === 7 && usef_sub.includes("gus-spd") ){
         
			max_score = max_score + Math.floor(substat_max[idx] * 2);
			continue;
		}
        if (idx === 8 && usef_sub.includes("gus-atkflat") ){
           
			max_score = max_score + Math.floor(substat_max[idx] / 10 );
			continue;
		}
        if (idx === 9 && usef_sub.includes("gus-defflat") ){
            
			max_score = max_score + Math.floor(substat_max[idx] / 7 );
			continue;
		}
        if (idx === 10 && usef_sub.includes("gus-hpflat") ){
			
			max_score = max_score + Math.floor(substat_max[idx] / 50 );
			continue;
		}
	}
	...
	if ( percent_score >= 70 && getGearEncLevel() > 4 ) {
		...
		if ( g_lang === 'tw' )
			str = str + "[總評] 神裝! 記得鎖起來! <br>";
        else if ( g_lang === 'cn' )
			str = str + "[总评] 神装! 记得锁起来! <br>";
		else 
		...
		if ( g_lang === 'tw' ) 
			str = str + "[總評] 還不錯，建議留下。<br>";
        else if ( g_lang === 'cn' ) 
			str = str + "[总评] 还不错，建议留下。<br>";
		else 
	...
	if ( g_lang === 'tw' ) {
		...
		for ( var idx = 0; idx < valid_data_size; idx ++ ) {
            if (valid_data_type[idx] === 0 && !usef_sub.includes("gus-atkper") ){
				continue;
			}
			if (valid_data_type[idx] === 1 && !usef_sub.includes("gus-defper") ){
				continue;
			}
			if (valid_data_type[idx] === 2 && !usef_sub.includes("gus-hpper") ){
				continue;
			}
			if (valid_data_type[idx] === 3 && !usef_sub.includes("gus-eff") ){
				continue;
			}
			if (valid_data_type[idx] === 4 && !usef_sub.includes("gus-res") ){
				continue;
			}
			if (valid_data_type[idx] === 5 && !usef_sub.includes("gus-critdmg") ){
				continue;
			}
			if (valid_data_type[idx] === 6 && !usef_sub.includes("gus-critch") ){
				continue;
			}
			if (valid_data_type[idx] === 7 && !usef_sub.includes("gus-spd") ){
				continue;
			}
			if (valid_data_type[idx] === 8 && !usef_sub.includes("gus-atkflat") ){
				continue;
			}
			if (valid_data_type[idx] === 9 && !usef_sub.includes("gus-defflat") ){
				continue;
			}
			if (valid_data_type[idx] === 10 && !usef_sub.includes("gus-hpflat") ){
				continue;
			}
			// the maximum substate for lv85 speed could be 5, but it extremely rare.
			// so we use 4 as maximum for the report
			...
			str = str + getSubstatName( valid_data_type[idx] ) +  "，共得到" + 
				  valid_data[idx] + "完美值为" + 
				  substat_max[valid_data_type[idx]];
